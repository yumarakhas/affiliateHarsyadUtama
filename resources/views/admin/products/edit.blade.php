...
<div class="p-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold text-gray-900">
                    Edit Varian Produk: {{ $product->name }}
                </h2>
                <p class="text-gray-600 mt-1">Edit informasi varian produk untuk carousel di halaman utama</p>
                ...
                <div class="mt-2 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                    ...
                    <div class="w-2 h-2 rounded-full mr-2 
                                {{ $product->category == 'gentle-baby' ? 'bg-blue-400' : 
                                   ($product->category == 'mamina' ? 'bg-pink-400' : 
                                   ($product->category == 'nyam' ? 'bg-orange-400' : 'bg-gray-400')) }}"></div>
                    Kategori: 
                    @if($product->category == 'gentle-baby') Gentle Baby
                    @elseif($product->category == 'mamina') Mamina  
                    @elseif($product->category == 'nyam') Nyam! MPASI
                    @else {{ ucfirst($product->category) }}
                    @endif
                </div>
            </div>
            <a href="{{ route('admin.content.carousel-varian') }}{{ $product->category ? '?category=' . $product->category : '' }}" 
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.content.carousel-varian.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
